...
import { isIngestionRunning } from "@/lib/ingestion-run";
...
export default async function Feed({ searchParams }: FeedProps) {
  ...
  const [feed, ingestionRunning] = await Promise.all([
    getFeedPage(session.user.id, page),
    isIngestionRunning(),
  ]);
  ...
  return (
    <main className="page-enter flex-1 py-7 sm:py-10">
      ...
      <div className="mx-auto w-full max-w-[1080px] px-6 pt-8">
        {ingestionRunning ? <IngestionNotice /> : null}
        {feed.articles.length > 0 ? (
          <FeedList articles={feed.articles} />
        ) : ingestionRunning ? (
          <EndMark title="Tuning your wire">
            Pulling fresh sources and ranking stories now.
          </EndMark>
        ) : (
...
function IngestionNotice() {
  return (
    <div
      className="ingestion-notice mb-7 flex items-center gap-3 border-y border-fg px-1 py-3"
      aria-live="polite"
    >
      <span aria-hidden="true" className="ingestion-pulse size-2 rounded-full bg-fg" />
      <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-fg">
        Tuning your wire{" "}
        <span className="text-meta">
          · pulling fresh sources and recalculating ranks
        </span>
      </p>
    </div>
  );
...
function Pagination({ page, hasNextPage }: { page: number; hasNextPage: boolean }) {
  ...
  return (
    ...
    >
      ...
      {hasNextPage ? (
        <Link href={`/feed?page=${page + 1}`} className={linkClasses}>
          Next →
        </Link>
      ) : (
        <span />
